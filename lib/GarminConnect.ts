...
import Running from 'garmin-connect/dist/garmin/workouts/Running';
import { checkIsDirectory, createDirectory, writeToFile } from 'garmin-connect/dist/utils';
import { toDateString } from 'garmin-connect/dist/garmin/common/DateUtils';
...
export default class GarminConnect {
    ...
    async getActivitiesForUser(user: string, start: number, limit: number): Promise<IActivity[]> {
        return this.client.get<IActivity[]>('https://connectapi.garmin.com/activitylist-service/activities/search/activities', {
            params: { start, limit }
        });
    }
